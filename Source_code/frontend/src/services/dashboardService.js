...
export const getToTruongDashboardSummary = async () => {
  try {
    ...
    // Filter out households with less than 1 member (population density must be 1 or more)
    const validHouseholds = households.filter(h => h.numMembers >= 1);
    
    const householdMemberData = [
      { name: '1 người', count: validHouseholds.filter(h => h.numMembers === 1).length },
      { name: '2-3 người', count: validHouseholds.filter(h => h.numMembers >= 2 && h.numMembers <= 3).length },
      { name: '4-5 người', count: validHouseholds.filter(h => h.numMembers >= 4 && h.numMembers <= 5).length },
      { name: '6+ người', count: validHouseholds.filter(h => h.numMembers >= 6).length }
    ];
    
    // Calculate average people per household (only count households with at least 1 member)
    const averagePeoplePerHousehold = validHouseholds.length > 0 
      ? Math.round((totalPeople / validHouseholds.length) * 100) / 100 
      : 0;

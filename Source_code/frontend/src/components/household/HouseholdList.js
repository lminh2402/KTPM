...
const HouseholdList = () => {
  ...
  // Refresh when returning to this page from detail page
  useEffect(() => {
    const currentPath = location.pathname;
    const prevPath = prevLocationRef.current;

    // If we're on the households list page and came from a detail page
    if (currentPath === '/households' && prevPath && prevPath.startsWith('/households/') && prevPath !== '/households') {
  ...
  }, [location.pathname, fetchHouseholds]);

  // Listen for custom refresh event (can be triggered from detail page)
  useEffect(() => {
    ...
    window.addEventListener('householdMembersUpdated', handleRefreshEvent);
    return () => {
      window.removeEventListener('householdMembersUpdated', handleRefreshEvent);
    };
  }, [location.pathname, fetchHouseholds]);

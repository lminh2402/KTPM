...
import { getAllTemporaryResidences, getTemporaryResidenceByPerson } from '../../services/temporaryResidenceService';
...
const PersonDetail = () => {
  ...
  // Function to load temporary residence records
  const fetchTemporaryResidences = React.useCallback(async (personId) => {
    try {
      // Try to use the person-specific endpoint first
      try {
        const personTempResidences = await getTemporaryResidenceByPerson(personId);
        setTemporaryResidences(Array.isArray(personTempResidences) ? personTempResidences : []);
      } catch (err) {
        // Fallback to getting all and filtering
        console.warn('Person-specific endpoint failed, using fallback:', err);
        const tempResidences = await getAllTemporaryResidences();
        const personTempResidences = tempResidences.filter(tr => tr.nhanKhauId === parseInt(personId));
  ...
  useEffect(() => {
    const fetchPersonData = async () => {
      ...
      try {
        ...
        if (id) {
          await fetchTemporaryResidences(id);
        }
      } catch (err) {
  ...
  // Listen for temporary residence updates
  useEffect(() => {
    if (!id) return;
    
    const handleTemporaryResidenceUpdate = (event) => {
      const { personId } = event.detail || {};
      // Refresh if the update is for this person or if no personId is specified (refresh all)
      if (!personId || personId.toString() === id.toString()) {
        fetchTemporaryResidences(id);
      }
    };

    window.addEventListener('temporaryResidenceUpdated', handleTemporaryResidenceUpdate);
    return () => {
      window.removeEventListener('temporaryResidenceUpdated', handleTemporaryResidenceUpdate);
    };
  }, [id, fetchTemporaryResidences]);

  // Refresh temporary residences when returning to this page from temporary residence form
  useEffect(() => {
    if (!id) return;
    ...
    // If we're on the person detail page and came from temporary residence form/list
    if (currentPath === `/persons/${id}` && prevPath && 
        (prevPath.startsWith('/temporary-residence') || prevPath.includes('temporary'))) {
  ...
  }, [location.pathname, id, fetchTemporaryResidences]);

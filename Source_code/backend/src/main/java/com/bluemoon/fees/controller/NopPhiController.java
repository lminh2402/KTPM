...
public class NopPhiController {
    ...
    public ResponseEntity<Map<String, Object>> getNopPhiStatistics(
        ...
        if (hoKhauId != null) {
            ...
            // Default: total of all verified payments (using soTien - actual amount paid, not tongTien - requested amount)
            List<NopPhi> allPayments = nopPhiService.findAll();
            totalAmount = allPayments.stream()
                    .filter(NopPhi::isDaXacNhan)  // Only count verified payments
                    .mapToDouble(NopPhi::getSoTien)  // Use soTien (actual amount paid), not tongTien (requested amount)
                    .sum();
            filterType = "all";
        }
